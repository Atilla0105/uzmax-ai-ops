...
test("captures owner source and React tenant config source parity refresh", async ({
  ...
  await expect(rollbackModal).toContainText("no production config write");
  await expect(rollbackModal).toContainText("no audit write");
  await expectLocalizedConfirm(rollbackModal, "回滚原因");
  await expect(rollbackModal.getByRole("button", { name: "回滚" })).toBeDisabled();
  await rollbackModal.getByRole("textbox").fill("local rollback reason");
  await rollbackModal.getByRole("button", { name: "回滚" }).click();
  await expectLocalToast(page, ["no audit write", "no production config write"]);
  ...
  await expectLocalToast(page, ["no API call", "no audit write"]);
  await page.getByRole("button", { name: "切换为导入快照主路径" }).click();
  const switchModal = page.getByTestId("m7-confirm-modal");
  await expect(switchModal).toContainText("no connector switch");
  await expect(switchModal).toContainText("no audit write");
  await expectLocalizedConfirm(switchModal, "切换原因");
  await saveShot(page, "react-config-connector-confirm-modal.png", true);
  await switchModal.getByRole("textbox").fill("local connector switch reason");
  await switchModal.getByRole("button", { name: "确认本地切换" }).click();
  await expectLocalToast(page, ["no connector switch", "no audit write"]);
