...
export function TeamStatePanel({ state, children }: { state: string; children: string }) {
  return <main aria-description={TEAM_RUNTIME_BOUNDARY} className="uz-team-state" data-runtime-boundary={TEAM_RUNTIME_BOUNDARY} data-testid={`m7-team-state-${state}`} title={TEAM_RUNTIME_BOUNDARY}><p>{children}</p><span hidden>{TEAM_RUNTIME_BOUNDARY}</span></main>;
}
