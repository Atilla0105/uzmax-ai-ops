...
Cleanly rebuild old PR #201 `M7-UI-59 sidebar icon density parity` as a narrow v2 slice based on `origin/codex/m7-ui-32-knowledge-resources-visible-ui-v2` at `05f23fe9e20a8826d08c9bd5e36534509e210b87`.

This slice calibrates the shared `AppShell` sidebar icon treatment, row density, active row treatment and bottom collapse-control geometry against the owner prototype and `/Users/atilla/源码/unpacked 6` shell sources. It does not rescue the old #200-#238 stack, does not merge or cherry-pick old branches, and does not change glyph mapping, routing, page bodies, runtime/API/DB/authz behavior, package dependencies, CI/global config, owner HTML or unpacked source.

## Owner Confirmation Points And AI Agent Responsibilities

Owner/coordinator:

- Review this as a small visible UI PR on the new v2 baseline, not a redesign or old-stack cleanup.
...
- Work only in `/Users/atilla/.codex/worktrees/m7-ui-59-sidebar-icon-density-parity-v2` on branch `codex/m7-ui-59-sidebar-icon-density-parity-v2`.
...
- Inspect old #201 head `43e5f35f131716f967c8b648c0acc08b2b50c50a` only for the six allowed paths; do not copy unrelated topology.
- Record owner HTML / unpacked source / React browser comparison evidence under `/tmp/uzmax-m7-ui-59-sidebar-icon-density-parity-v2/`.
...
- `npm run guard:pr-shape -- --base origin/codex/m7-ui-32-knowledge-resources-visible-ui-v2 --spec docs/specs/M7-UI-59-shared-sidebar-icon-treatment-parity.md --include-worktree`
- `npm run typecheck`
- `npm run lint`
- `npm run build:admin`
