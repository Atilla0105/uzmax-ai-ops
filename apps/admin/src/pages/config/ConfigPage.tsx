...
const sectionViews: Record<ConfigSection, (props: ViewProps) => ReactNode> = {
  ...
  connector: ({ connectorSwitch, draft, state }) => { const c = draft.connector; const isApi = c.mode === "api"; return <section className="uz-config-panel uz-config-connector"><Kv k="当前主路径" v={isApi ? "订单 API（本地主路径）" : "导入快照（本地主路径）"} /><Kv k="订单 API 健康度" v={c.apiHealth} /><Kv k="最近错误" v={c.lastError} /><Kv k="最近同步" v={c.lastSync} /><div className="uz-config-actions"><button className="uz-config-mini" onClick={state.update.connectorTest} type="button">测试连接</button><button className="uz-config-danger" onClick={() => connectorSwitch(isApi ? "import" : "api")} type="button">{isApi ? "切换为导入快照主路径" : "切换回订单 API 主路径"}</button><button className="uz-config-link" type="button">查看导入历史（本地预览）</button></div></section>; },
...
export function ConfigPage({ selectedTenantId }: { selectedTenantId: string }) {
  ...
  return (
    <section className="uz-config-page" data-runtime-source={configMeta.source} data-runtime-state={state.viewState} data-testid="m7-config-page">
      <style>{configStyles}</style>
      <SideNav section={state.section} setSection={state.setSection} />
      <section className="uz-config-main">
        <header className="uz-config-head"><div className="uz-config-head-row"><h2 className="uz-config-title">{configSectionLabels[state.section]}</h2><span className="uz-config-meta" data-testid="m7-config-version-meta">当前版本 v{state.version.ver} · {versionCopy(state)}</span><StatusBadge tone="warn">{selectedTenantId} · tenant layer</StatusBadge>{dirty ? <StatusBadge tone="warn">未保存的修改</StatusBadge> : null}<div className="uz-config-tools">{state.sectionHistory.length ? <button className="uz-config-mini" onClick={() => state.setHistoryOpen(historyOpen ? null : state.section)} type="button">{historyOpen ? "收起版本历史" : `版本历史（${state.sectionHistory.length}）`}</button> : null}{state.isDirtyable ? <button className="uz-config-btn is-primary" data-testid="m7-config-save" disabled={!dirty} onClick={state.save} type="button">保存并生成版本</button> : null}</div></div></header>
        <RuntimeNote />
        <Toast message={state.toast} />
        {state.isDegraded ? <main className="uz-config-body"><p className="uz-config-muted" data-testid="m7-config-source-mark">{configMeta.source} · {configMeta.subtitle}</p>{historyOpen ? <History history={state.sectionHistory} onOpenRollback={openRollback} /> : null}{sectionViews[state.section]({ connectorSwitch: openSwitch, draft: state.draft, state })}</main> : <main className="uz-config-state" data-testid={`m7-config-state-${state.viewState}`}><div><h2>{state.viewState === "permission" ? "permission denied" : state.viewState}</h2><p>{state.stateCopy}</p></div></main>}
      </section>
...
function SideNav({ section, setSection }: { section: ConfigSection; setSection: (section: ConfigSection) => void }) {
  return <aside className="uz-config-side" data-testid="m7-config-internal-nav"><h2>配置</h2><nav aria-label="配置段" className="uz-config-nav">{configSections.map((item) => <button aria-pressed={item.id === section} key={item.id} onClick={() => setSection(item.id)} type="button">{item.label}</button>)}</nav></aside>;
}
...
function History({ history, onOpenRollback }: { history: readonly ConfigVersion[]; onOpenRollback: (version: ConfigVersion) => void }) {
  return <section className="uz-config-history" data-testid="m7-config-history"><div className="uz-config-history-head">版本历史 · 回滚原因必填 · browser-local only</div>{history.map((item) => <div className="uz-config-history-row" key={item.ver}><span className="uz-config-version">v{item.ver}</span><div className="uz-config-row-main"><div className="uz-config-row-title">{item.note}</div><div className="uz-config-row-sub">{item.by} · {item.at} · no audit write</div></div><button className="uz-config-danger" onClick={() => onOpenRollback(item)} type="button">回滚到此版本</button></div>)}</section>;
}
