...
Visible UI refresh candidate evidence for `tenant.customers` / 客户资产 on clean-stack branch `codex/m7-ui-65-customer-assets-source-parity-refresh-cleanstack`, based on `origin/codex/m7-ui-31-orders-visible-ui` (#251/current visible UI trunk).
...
| Fact | Evidence |
|---|---|
| worker path | `/Users/atilla/.codex/worktrees/m7-ui-65-customer-assets-source-parity-refresh-cleanstack` |
| worker branch | `codex/m7-ui-65-customer-assets-source-parity-refresh-cleanstack` |
| worker status at entry | `## codex/m7-ui-65-customer-assets-source-parity-refresh-cleanstack...origin/codex/m7-ui-31-orders-visible-ui` |
| entry HEAD | `514a5cc7025e883b228a9d526926ba32d977542a` |
| base | `origin/codex/m7-ui-31-orders-visible-ui` / #251 current visible UI trunk |
...
| Path | Summary |
|---|---|
| `docs/specs/M7-UI-65-customer-assets-source-parity-refresh.md` | Adds scoped source-parity refresh spec for customer assets on the current visible UI trunk. |
...
- Owner/source unavailable artifact when local file is missing: `/tmp/uzmax-m7-ui-65-customer-assets-source-parity-refresh/owner-html-customer-assets-source-dom-sample.json`
...
- Unpacked source unavailable artifact when local files are missing: `/tmp/uzmax-m7-ui-65-customer-assets-source-parity-refresh/unpacked-customer-assets-source-mapping-unavailable.json`
...
| `npm run format:check` | pass | Full repo format check passed. |
| `npm run jscpd` | pass | Full duplicate-code guard passed after making the focused customer-assets parity test distinct from the ticket parity test. |
| `npm run knip` | pass | Full unused exports/dependencies guard passed. |
| `npm run guard:pr-shape -- --base origin/codex/m7-ui-31-orders-visible-ui --spec docs/specs/M7-UI-65-customer-assets-source-parity-refresh.md --include-worktree` | pass | Final post-commit guard reports `changedFiles: 5`, categories `docs: 4`, `test: 1`, source `changedFiles: 0`, `netLoc: 0`, `newFiles: 0`. |
| `npx playwright test apps/admin/tests/m7-ui-customer-assets-source-parity.spec.ts apps/admin/tests/m7-ui-customer-assets.spec.ts` | pass | 6/6 passed; writes owner/source sample, unpacked mapping, desktop list/detail/collapsed/mobile screenshots and metrics under `/tmp/uzmax-m7-ui-65-customer-assets-source-parity-refresh/`. |
| `npm run typecheck` | pass | Full repo typecheck passed after local `prisma generate --schema packages/db/prisma/schema.prisma` restored ignored generated client in this worktree. |
| `npm run lint` | pass | Full repo lint passed. |
