...
| UI-05 | M7-UI-05 Layered Navigation Shell | `merged_to_main` | `docs/specs/M7-UI-05-layered-navigation-shell.md`; `docs/evidence/M7/M7-UI-05-layered-navigation-shell.md`; merged via PR #180 / commit `8804414`; focused shell/router tests, legacy `/design` route-entry migration tests and screenshots for group-only vs tenant-only AppShell navigation |
...
| UI-11 | M7-UI-11 Release Acceptance Page | `implementation_pending_pr_review` | `docs/specs/M7-UI-11-release-acceptance-page.md`; `docs/evidence/M7/M7-UI-11-release-acceptance-page.md`; focused Playwright `apps/admin/tests/m7-ui-release-acceptance.spec.ts`; PR pending, not merged/main |
| UI-11A | M7-UI-11 Worker Boundary Incident | `merged_to_main` | `docs/specs/M7-UI-11A-worker-boundary-incident.md`; `docs/evidence/M7/M7-UI-11A-worker-boundary-incident.md`; `docs/incidents/INC-2026-07-03-m7-ui-11-root-patch-target.md`; merged via PR #177 / commit `da33a08` |
| UI-04A+ | Page migration implementation queue | `page_specific_specs_required` | UI-05 and UI-10 are merged, UI-11 is implementation-pending PR review on the layered AppShell; subsequent page implementations still require approved page-specific specs, runtime contracts, state matrices, tests and evidence |
...
M7-UI-00 through M7-UI-02 establish the migration index, fixture safety, token/foundation contract, shared foundation layer and shell icon calibration on `main`. M7-UI-03 adds only the permanent page ledger, typed admin page registry and router/page-outlet scaffold. M7-UI-04 adds shared operational patterns for later page workers. M7-UI-05 is merged to `main` via PR #180 / commit `8804414`: group pages render group-only nav, tenant pages render tenant-only nav, admin/home starts at group layer, selecting a tenant enters tenant layer, and older legacy evidence Playwright specs explicitly click the legacy evidence route instead of expecting legacy panels at `/design` initial load. M7-UI-10 is merged to `main` for `tenant.queue` via PR #175 / commit `c82fa4d`; it does not close GA-0, M7, M5 owner acceptance, production, release, distill health recovery API, or keep-current runtime semantics. M7-UI-11 is implementation-pending PR review for the owner-facing `group.release` release/acceptance console on the layered AppShell; it renders read-only/degraded release evidence and keeps GA-0/1.0 actions disabled, but it is not merged/main and does not approve GA-0/1.0. M7-UI-11A records the M7-UI-11 implementation worker root/main patch-target incident and is merged to `main` via PR #177 / commit `da33a08`.
