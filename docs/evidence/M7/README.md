...
| REQ-G01A-code-01 | Group Overview DB/API Foundation | `code_01_db_api_foundation_implemented_not_admin_wired_not_visual_accepted` | `docs/specs/REQ-G01A-code-01-group-overview-db-api-foundation.md`; `docs/evidence/M7/REQ-G01A-code-01-group-overview-db-api-foundation.md`; adds the guarded API contract/controller/service/runtime/repository skeleton with disabled default, per-authorized-tenant RLS repository foundation and focused true DB/RLS smoke harness; detailed last-abnormal event mapping plus available-source deterministic provenance for `llm_call_log` cost/model fault, `channel_connection.status` and order source health are omitted and represented as `source_partial`/bounded gaps; true DB/RLS smoke result is `not_run_missing_UZMAX_RLS_DATABASE_URL`, so no true-DB pass, DB migration, Prisma schema change, admin wiring, populated rendering, visual acceptance, owner acceptance or release approval is claimed |
