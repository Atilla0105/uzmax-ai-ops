...
Refresh the existing visible UI-first `tenant.orders` / 订单 page on top of `origin/codex/m7-ui-65-customer-assets-source-parity-refresh` (#207 stack) so the orders page remains browser-comparable against the owner HTML, frozen unpacked orders source and latest stacked tenant shell.

This is a source parity refresh, not a rewrite. Primary scope is evidence/test/docs only. Small `apps/admin/src/pages/orders/**` corrections are allowed only if browser evidence proves an obvious current React mismatch that must be fixed now. This slice does not implement order DB/API/runtime, does not read or parse real CSV/XLSX files, does not call an external order API, and does not claim owner visual acceptance, runtime closure, GA-0, production deployment, real customer/order-data use, customer LLM, Telegram Business automatic reply or 1.0 release approval.
...
- Confirm this branch is stacked on #207 / `origin/codex/m7-ui-65-customer-assets-source-parity-refresh`.
...
- Work only in `/Users/atilla/.codex/worktrees/m7-ui-66-orders-source-parity-refresh` on branch `codex/m7-ui-66-orders-source-parity-refresh`.
...
0.5 workday. If the page requires backend/API/DB/packages/package/lock/CI/global config/shared AppShell/topbar/sidebar/registry/PageOutlet edits, a broad page rewrite, raw owner fixture import, production data, external API behavior or release action, stop and report `BLOCKED`.
...
- source changed files: 0 expected; <= 2 conditional only
- source net LOC: 0 expected; <= 80 conditional only
- new source files: 0
...
- Owner/source screenshot and DOM/text sample for orders-related owner HTML region.
- Unpacked source mapping summary for orders page/hook/fixtures.
...
- `tenant.orders` renders inside tenant shell after selecting tenant `tenant-b` on the latest #207 stack.
...
- Any React visual corrections are small and limited to `apps/admin/src/pages/orders/**`.
- No disallowed files are changed.

## Validation Plan

- `git diff --check origin/codex/m7-ui-65-customer-assets-source-parity-refresh...HEAD`
- `node scripts/guards/pr-shape.mjs --base origin/codex/m7-ui-65-customer-assets-source-parity-refresh --spec docs/specs/M7-UI-66-orders-source-parity-refresh.md --include-worktree`
- touched Prettier check
- ESLint on touched orders test/source if source changed
...
- Focused Playwright for `apps/admin/tests/m7-ui-orders-source-parity.spec.ts` and existing `apps/admin/tests/m7-ui-orders.spec.ts`.
