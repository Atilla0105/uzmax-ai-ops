...
| D-04 | `admin_api_client_supported_not_closed` | M4-02 adds customer, identity, field and tag persistence foundation; M4-14 adds customer list/detail API shell with controlled related refs; M4-15 adds admin client contract. Full UI/history/order/quote/ticket aggregation remains future scope. |
| D-05 | `admin_api_client_supported_not_closed` | M4-02 adds blacklist/unreachable flags and timestamps; M4-14 adds restore API shell with audit draft; M4-15 validates admin restore request/response shape. Admin owner flow and real audit persistence remain future scope. |
| D-07 | `admin_api_client_supported_not_closed` | M4-02 adds customer field and customer tag definitions/assignments; M4-14 exposes field/tag definition API shell; M4-15 adds admin client contract for definitions. Conversation tags/admin config/analysis reuse remains future scope. |
...
| I-01 | `partial_admin_client_not_closed` | M4-01 adds visible admin order-path status shell, M4-06 extends import/search states, M4-13 adds the order import admin API client contract and M4-15 adds the customer asset admin API client contract; full desktop core order/customer workflow remains future scope. |
