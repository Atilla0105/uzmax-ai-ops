...
- `ConfirmationQueueService` still persists the decision through the existing confirmation repository first, then calls an injected formal-write pipeline.
...
- The RLS Prisma formal-write pipeline uses repo-local `createRlsTransactionContext`, `set local role "uzmax_app_runtime"`, transaction-scoped `app.org_id` and `app.tenant_id`, then creates `config_version`, creates `audit_log`, and updates `confirmation_item` in the same RLS transaction.
...
Implementation source remains within M5R-02 budget: 8 changed source files, 3 new source files, and approximately +595 net source LOC for source-classified files.
...
Error: UZMAX_RLS_DATABASE_URL is required
    at requireSmokeEnv (.../packages/db/scripts/tests/run-m5r-formal-write-true-db-smoke.mjs:143:21)
```

The runner calls `requireSmokeEnv("UZMAX_RLS_DATABASE_URL")` before constructing `PrismaClient`, so the missing-env path does not open a DB connection or mutate data. Static/focused tests prove the same fail-closed behavior and the non-RLS/bare Prisma bypass rejection.
...
| Command | Result | Notes |
|---|---|---|
| `node --test scripts/tests/m5r-formal-write-pipeline.test.mjs` | pass | 5/5 tests after final source-budget trim. |
...
| `npm run test` | pass | 365/365 tests passed. |
...
- The formal-write port is disabled by default and injected, keeping existing M5-03/M5R-01 runtime behavior stable.
