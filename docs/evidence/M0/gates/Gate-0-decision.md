...
**No-Go / 独立项目部分完成，Supabase、Telegram、Render/Sentry 仍阻塞。**

原因：OCM-00 已按单 owner 模式闭合；GitHub repo、Vercel project 与 OpenAI dev key 已按独立命名创建/验证。Supabase `uzmax-dev` 创建因 free active project limit 2/2 失败；Telegram 测试 bot、Render/Sentry 仍需安全输入或登录。根据控制矩阵，Gate 0 不能跳过真实环境前置条件。
...
| G0-3 | partially_ready__blocked_by_supabase_telegram_render_sentry | `docs/evidence/M0/infra/*-manifest.md` | GitHub/Vercel/OpenAI key 已创建；Supabase free project limit、Telegram 测试 bot、Render/Sentry 登录仍阻塞 |
...
- 不启动 M0-01 的真实 CI/分支保护，不接真实业务流量。
- 不启动 SPK-03、SPK-04 或需要真实 Supabase dev 的验证项。
- 先完成 OCM-00A 的实际分支：处理 Supabase free project limit；补 Telegram 测试 bot；登录 Render/Sentry 或写顺延记录。
