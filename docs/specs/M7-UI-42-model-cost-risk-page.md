...
Implement a lean UI-first `group.modelRisk` / `模型/成本/风险` visible page on top of `origin/codex/m7-ui-41-ai-members-visible-ui-v2`.
...
- This is a GROUP layer page. It may enter tenant layer only through existing tenant ids `tenant-a` / `tenant-b` / `tenant-c` / `tenant-d`.
...
- Update `PageOutlet.tsx` to render `<GroupModelRiskPage onEnterTenant={onEnterTenant} />` for `group.modelRisk`; it must not carry `data-tenant-id`.
...
- Browser evidence under `/tmp/uzmax-m7-ui-42-model-cost-risk-visible-ui/` with desktop screenshot, mobile 320 screenshot and metrics JSON.
