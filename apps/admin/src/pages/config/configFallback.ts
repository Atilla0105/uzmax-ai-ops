...
export const routeRows = [
  ["意图识别", "gpt-4o-mini", "qwen-2.5", "¥1.2", "218ms", "0.4%"],
  ["对话生成", "gpt-4o", "claude-haiku", "¥9.6", "612ms", "0.9%"],
  ["报价", "gpt-4o", "gpt-4o-mini", "¥8.1", "540ms", "4.2%"],
  ["截图理解", "gpt-4o-vision", "—", "¥14.0", "1.1s", "2.1%"],
  ["红线检查", "内置分类器", "gpt-4o-mini", "¥0.3", "42ms", "0.1%"]
];
export const templateRows = [
  ["售后知识包", "v4.2", "v4.2", "06-26", "已同步"],
  ["默认配置", "v2.3", "v2.4", "05-30", "有更新"],
  ["红线评测集", "v5.0", "v5.0", "06-10", "已同步"],
  ["客服人设", "v3.0", "v3.1", "04-22", "本地修改"]
];
...
const initialDraft: ConfigDraft = {
  ...
  channels: [
    {
      desc: "标准机器人渠道 · AI 自动发送",
      enabled: true,
      id: "tg_bot",
      lastTest: "06-29 09:00",
      name: "Telegram Bot",
      status: "正常"
    },
    {
      desc: "人工号双轨 · AI 生成草稿，需人工确认发送",
      enabled: true,
      id: "tg_biz",
      lastTest: "06-28 17:20",
  ...
  guardrails: [
    { cap: "1,000", editable: false, k: "集团日预算", v: "800" },
    { cap: "200", editable: true, k: "本租户日预算", v: "120" },
    { cap: "", editable: true, k: "单会话上限", v: "1.5" }
  ],
  sla: [
    { first: "5", id: "p0", label: "P0 · 红线 / 投诉", resolve: "60" },
    { first: "15", id: "p1", label: "P1 · 一般转人工", resolve: "240" },
    { first: "60", id: "p2", label: "P2 · 低优先级咨询", resolve: "1440" }
  ]
};
const initialVersions: Record<ConfigSection, ConfigVersion> = {
  biz: { at: "06-20 14:10", by: "韩雪", ver: 3 },
  sla: { at: "06-18 09:30", by: "李伟", ver: 5 },
  tmpl: { at: "来自集团模板中心", by: "模板来源", ver: 1 },
  model: { at: "今天 10:42", by: "韩雪", ver: 17 },
  cost: { at: "06-29 09:58", by: "李伟", ver: 12 },
  breaker: { at: "06-15 11:20", by: "韩雪", ver: 4 },
  channel: { at: "06-25 16:00", by: "李伟", ver: 8 },
  connector: { at: "06-22 08:40", by: "韩雪", ver: 6 }
};
const initialHistory: Partial<Record<ConfigSection, ConfigVersion[]>> = {
  biz: [
    { at: "06-01 10:00", by: "韩雪", note: "调整默认时区为塔什干", ver: 2 },
    { at: "05-10 09:00", by: "韩雪", note: "初始业务配置", ver: 1 }
  ],
  cost: [{ at: "06-20 10:00", by: "李伟", note: "本租户日预算上调到 120", ver: 11 }],
  model: [
    { at: "06-28 14:00", by: "韩雪", note: "报价任务主模型切到 gpt-4o", ver: 16 }
  ],
  sla: [{ at: "05-20 09:00", by: "李伟", note: "P0 首次响应收紧到 5 分钟", ver: 4 }]
};
...
export function useConfigPageState() {
  ...
  const markDirty = (target: ConfigSection) =>
    setDirty((value) => ({ ...value, [target]: true }));
  const save = () => {
    if (!dirty[section]) return;
    const current = versions[section];
    setHistory((value) => ({
      ...value,
      [section]: [
        { ...current, note: `${configSectionLabels[section]}变更预览` },
        ...(value[section] ?? [])
      ]
    }));
    setVersions((value) => ({
      ...value,
      [section]: { at: "刚刚", by: "当前操作员", ver: current.ver + 1 }
    }));
    setDirty((value) => ({ ...value, [section]: false }));
    showToast("配置变更已暂存，并生成新的版本预览");
  };
  const rollback = (target: ConfigVersion) => {
    setVersions((value) => ({
      ...value,
      [section]: { at: "刚刚", by: "当前操作员", ver: value[section].ver + 1 }
    }));
    setDirty((value) => ({ ...value, [section]: false }));
    showToast(`回滚到 v${target.ver} 的预览已排队`);
  };
  const update = {
    biz: (key: keyof ConfigDraft["biz"], value: string) => {
      setDraft((d) => ({ ...d, biz: { ...d.biz, [key]: value } }));
      markDirty("biz");
    },
    breaker: (index: number, value: string) => {
      setDraft((d) => ({
        ...d,
        breakers: d.breakers.map((row, i) => (i === index ? { ...row, v: value } : row))
      }));
      markDirty("breaker");
    },
    channelTest: (id: string) => {
      setDraft((d) => ({
        ...d,
        channels: d.channels.map((row) =>
          row.id === id ? { ...row, lastTest: "刚刚", status: "检查通过" } : row
        )
      }));
      showToast("渠道连通性检查已更新");
    },
    channelToggle: (id: string) => {
      setDraft((d) => ({
        ...d,
        channels: d.channels.map((row) =>
    ...
    connectorSwitch: (mode: ConfigConnector["mode"]) => {
      setDraft((d) => ({ ...d, connector: { ...d.connector, mode } }));
      showToast("订单数据主路径预览已切换");
    },
    connectorTest: () => {
      setDraft((d) => ({ ...d, connector: { ...d.connector, lastSync: "刚刚" } }));
      showToast("订单连接健康状态已刷新");
    },
    guard: (index: number, value: string) => {
      setDraft((d) => ({
        ...d,
        guardrails: d.guardrails.map((row, i) =>
          i === index ? { ...row, v: value } : row
        )
      }));
      markDirty("cost");
    },
    sla: (id: string, key: "first" | "resolve", value: string) => {
      setDraft((d) => ({
        ...d,
        sla: d.sla.map((row) => (row.id === id ? { ...row, [key]: value } : row))
      }));
      markDirty("sla");
    }
  };
  ...
  return {
    ...
    isDirtyable: dirtyable.includes(section),
    rollback,
    save,
    section,
    sectionHistory,
    setHistoryOpen,
    setSection,
    stateCopy: configStateCopy[viewState],
    toast,
    update,
