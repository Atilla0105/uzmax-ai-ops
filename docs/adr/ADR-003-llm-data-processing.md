...
> 状态：proposed__blocked_for_real_customer_traffic
> 日期：2026-06-14
...
M0-04 的当前结论是：**ADR 规则可作为后续实现约束，但真实客户消息、截图、语音转写和客户档案不得进入第三方 LLM，直到项目 owner 对 provider 留存/区域/数据共享状态签收，且 `packages/llm-gateway` 已实现 redaction、trace 和 fallback 策略。**

默认策略如下：

- fallback provider 默认关闭；provider 全挂或 OpenAI 不满足策略时，系统只能转人工、留单或关闭对应 AI 能力，不得切到更宽松 provider。
- OpenAI 仅允许用于合成数据、脱敏开发样本、公开知识和非客户明文任务；真实客户内容必须等 redaction 实装与 owner signoff。
...
| `dev_only` | 只有 API key 与公开政策证据，没有 owner signoff 或 redaction 实装。 | 仅允许合成/脱敏开发样本和非客户明文任务。 | 当前状态；真实流量不开闸。 |
...
- 签收状态：项目 owner 未签收真实客户数据进入第三方 LLM；当前状态保持 `proposed__blocked_for_real_customer_traffic`。
