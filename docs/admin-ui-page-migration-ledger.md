...
| 0.5 | M7-UI-05-layered-navigation-shell | Layered AppShell/navigation foundation | `shell/AppShell.tsx`, `shell/navigation.ts`, `shell/NavSidebar.tsx`, `shell/TopBar.tsx`, owner HTML shell | `level: group \| tenant`; admin/home -> `group.overview`; tenant selection -> tenant route | future `apps/admin/src/shell/AppShell.tsx`, `apps/admin/src/shell/AppShell.css`, `apps/admin/src/pages/registry.ts`, `apps/admin/src/App.tsx` | `spec_ready_pending_pr_review`; current AppShell renders mixed group+tenant sidebar, blocking broad page migration and UI-11 full visual/IA parity until future implementation lands | group-only nav, tenant-only nav, tenant selection transition, back-to-group, 320px fallback, no mixed accessible nav | `spec_evidence_stub_pending_pr_review` |

## Planned Page Ledger

Current ledger state is mixed: M7-UI-10 is merged to `main`, M7-UI-05 layered navigation shell is spec-ready pending PR review as a foundation blocker, M7-UI-11 is spec-ready pending PR review, and the remaining planned pages are not implemented. Any later page worker must update this ledger or a page-specific evidence file before claiming implementation.
