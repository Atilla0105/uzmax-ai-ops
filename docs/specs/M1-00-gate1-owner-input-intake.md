...
把 Gate 1 当前唯一未闭合的 owner input 变成可执行、可复判、可失败分支的补充包：历史真实咨询样本导出与脱敏复核路径，或明确顺延结论、受控存储位置与访问方式、抽样脱敏检查许可。本 spec 只更新 Gate 1 输入状态与复判准备，不放行 M1 实现。
...
0.25 个工作日。若本 PR 合并时仍未拿到可签收的脱敏样本、受控存储/访问范围、抽样脱敏复核结论或明确顺延结论，则 Gate 1 保持 No-Go，只记录下一步 owner 决策缺口。
...
2. 更新 history samples manifest，明确补齐 Gate 1 的两条最终合法分支和一个中间复核状态：
   - `raw_real_export_available` / `redacted_seed_review_ready`：项目 owner 提供本地真实 Telegram 导出并同意按建议走；AI agent 只在本机只读解析，生成仓库外脱敏候选与 seed review 文件；所有记录仍为 `needs_human_review`，不等同于 `sample_ready`。
   - `sample_ready`：提供脱敏样本、受控存储、抽样检查许可。
   - `sample_deferred`：明确样本不可按期提供，顺延 M1 eval seed 与 M2/M3 智能验收，只允许准备或执行不消费真实样本的平台骨架工作。
3. 更新 Gate 1 decision，记录 M1-00 的本地脱敏复核结果与后续动作；在没有人工复核签收或明确顺延前保持 No-Go。
4. 运行格式、doc trigger、PR shape 和本地验证命令。

## 通过条件

- `docs/evidence/M1/eval-seed/history-samples-manifest.md` 写清补充字段、分支判定、本地仓库外脱敏复核状态和不得提交客户明文的规则。
...
- 若本地脱敏候选仍未完成人工复核：保持 `redacted_seed_review_ready`，不得把候选写入正式 eval seed，不得关闭 G-06，不得放行 M1 业务骨架。
