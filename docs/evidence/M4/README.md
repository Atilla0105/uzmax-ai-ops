...
| E-02 | `p0_current_main_path__import_snapshot` | CSV/table import snapshot is the current order-data main path. M4-04 adds DB contracts, M4-05 adds pure row/batch contracts, M4-06 adds synthetic admin visibility, M4-07 adds API shell visibility for import jobs/errors/search, M4-08 adds repository port/persistence mappers, and M4-09 adds worker-side controlled-row draft generation; real parser, DB client wiring, queue runtime and admin E2E remain future scope. |
| E-03 | `p0_remains__stale_snapshot_warning` | M4-04 adds expiry fields, M4-05 adds stale handoff contract, M4-06 adds admin stale warning shell, M4-07 adds stale API response contract, M4-08 preserves expiry/source fields through repository mapping, and M4-09 preserves expiry/source fields in worker snapshot drafts; persisted/runtime warning remains required. |
| E-04 | `p0_remains__no_fabricated_order_status` | M4-05 pure contract, M4-06 admin shell, M4-07 API shell, M4-08 repository mapping and M4-09 worker contract hand off missing/stale/degraded reads without status fabrication; AI eval/runtime integration remains future scope. |
...
- M4-01, M4-02, M4-04, M4-05, M4-06, M4-07, M4-08 and M4-09 are foundation/partial evidence only. They do not close the order import workflow runtime/admin/E2E blockers, real parser, DB repository/runtime wiring, worker integration beyond the pure M4-09 draft contract, DB client wiring, worker import queue runtime, stale snapshot runtime warning, customer asset API/admin flows, AI order-read runtime/eval, production readiness or release signoff.
