...
export function ThreadHeader({
  ...
  return (
    <header className="uz-conv-thread__head">
      ...
      <span
        className="uz-conv-header-caveat"
        data-testid="m7-conversation-degraded"
        title={degradedReason}
      >
        只读预览
        <span className="uz-conv-sr-only">
          degraded {degradedReason} 发送、人工作业外发、客户聚合和 WS 实时同步保持禁用。
        </span>
      </span>
      <div className="uz-conv-actions">
        ...
        <Button
          aria-label="更多会话动作待接入"
          disabled
          icon={<IconSlot icon={MoreHorizontal} />}
          title="runtime action contract pending"
        />
      </div>
    </header>
  );
...
function threadBadges(active?: ConversationRow) {
  if (!active) return [];
  return [
    active.status === "pending_handoff" && { label: "待人工", tone: "danger" },
    active.status === "handoff" && { label: "人工处理中", tone: "warn" },
    active.status === "closed" && { label: "已解决", tone: "ok" },
    active.status === "open" && { label: "AI 处理中", tone: "info" },
    active.slaRisk && { label: "SLA 即将超时", tone: "danger" },
    active.aiState === "suspended" && { label: "AI 已暂停", tone: "danger" }
  ...
  if (messages.length === 0)
    return (
      <div className="uz-conv-body">
        <PageState
          aria-description="runtime detail/messages/context missing"
          data-testid="m7-conversation-customer-context-unavailable"
          data-runtime-boundary="runtime detail/messages/context missing"
          kind="degraded"
          message="详情消息、AI 轨迹和客户上下文待接入。"
          title="客户上下文待接入"
        />
      </div>
    );
...
type ComposerProps = { active?: ConversationRow; degradedReason: string };
    ...
    <footer className="uz-conv-composer" data-testid="m7-conversation-composer">
      <div className="uz-conv-composer__state">
        <StatusBadge tone="warn">
          {suspended ? "人工外部回复 · 待确认" : "Business 草稿 · 待确认"}
        </StatusBadge>
        <span className="uz-conv-caveat" data-testid="m7-conversation-composer-caveat">
          由 AI 生成，确认后才会发送给客户
          <span className="uz-conv-sr-only">{degradedReason}</span>
        </span>
      </div>
      ...
      <div className="uz-conv-composer__bar">
        ...
        <Button disabled icon={<IconSlot icon={MessageSquareQuote} />}>
          话术片段
        </Button>
        <span className="uz-conv-language">
          {active?.language?.replace(" / mock", "") ?? "语言待识别"}
        </span>
