...
export const analyticsRuntimeBoundary = analyticsRuntimeLabels.join(" | ");
...
const analyticsStateCopy: Record<AnalyticsViewState, { body: string; title: string }> =
  {
    degraded: {
      body: "当前展示最近可用的分析概览，实时指标恢复后会自动刷新。",
      title: "分析概览暂未实时更新"
    },
    empty: {
      body: "当前时间范围内暂无可展示的会话、转人工或成本数据。",
      title: "暂无分析数据"
    },
    error: {
      body: "分析数据暂时无法读取，请稍后刷新或切换时间范围重试。",
      title: "分析数据暂不可用"
    },
    loading: {
      body: "正在汇总会话、转人工、成本和知识健康指标。",
      title: "正在加载分析"
    },
    permission: {
      body: "请联系管理员开通分析页查看权限后再访问。",
      title: "需要分析权限"
    }
  };
...
export function useAnalyticsPageState() {
  ...
  const toggleDim = (dim: AnalyticsDimension) => {
    setDims((value) => {
      ...
      if (value.length >= 2) {
        showToast("最多只能同时查看 2 个维度，请先移除一个维度。");
        return value;
      }
      return [...value, dim];
    });
  ...
  return {
    ...
    dims,
    exportLocal: () => showToast("导出预览已生成，正式任务将在接入后写入审计记录。"),
    handoffReasons,
    ...
    setMenuOpen,
    setRange,
    stateCopy: analyticsStateCopy[viewState],
    table,
    toast,
    toggleDim,
