...
export function useConfigPageState() {
  ...
  const save = () => { if (!dirty[section]) return; const current = versions[section]; setHistory((value) => ({ ...value, [section]: [{ ...current, note: `${configSectionLabels[section]}本地修改` }, ...(value[section] ?? [])] })); setVersions((value) => ({ ...value, [section]: { at: "刚刚", by: "browser-local", ver: current.ver + 1 } })); setDirty((value) => ({ ...value, [section]: false })); showToast("保存并生成本地版本；no production config write / no audit write"); };
  const rollback = (target: ConfigVersion) => { setVersions((value) => ({ ...value, [section]: { at: "刚刚", by: "browser-local", ver: value[section].ver + 1 } })); setDirty((value) => ({ ...value, [section]: false })); showToast(`本地回滚到 v${target.ver}；no audit write / no production config write`); };
  const update = {
    ...
    channelTest: (id: string) => { setDraft((d) => ({ ...d, channels: d.channels.map((row) => (row.id === id ? { ...row, lastTest: "刚刚", status: "本地通过" } : row)) })); showToast("渠道测试仅本地通过；no connector switch / no audit write"); },
    channelToggle: (id: string) => { setDraft((d) => ({ ...d, channels: d.channels.map((row) => (row.id === id ? { ...row, enabled: !row.enabled } : row)) })); markDirty("channel"); },
    connectorSwitch: (mode: ConfigConnector["mode"]) => { setDraft((d) => ({ ...d, connector: { ...d.connector, mode } })); showToast("订单主路径切换仅更新浏览器状态；no connector switch / no audit write"); },
    connectorTest: () => { setDraft((d) => ({ ...d, connector: { ...d.connector, lastSync: "刚刚" } })); showToast("connector 测试仅本地刷新；no API call / no audit write"); },
  ...
  return { dirty, draft, historyOpen, isDegraded: viewState === "degraded", isDirtyable: dirtyable.includes(section), rollback, save, section, sectionHistory, setHistoryOpen, setSection, stateCopy: `Synthetic ${viewState} state. ${configMeta.subtitle}.`, toast, update, version: versions[section], viewState };
}
