...
Refresh the existing visible UI-first `tenant.knowledge` / 知识与资源 page on top of `origin/codex/m7-ui-66-orders-source-parity-refresh` (#208 stack) so the knowledge/resources page remains browser-comparable against the owner HTML, frozen unpacked knowledge source and latest stacked tenant shell.

This is a source parity refresh, not a rewrite. Primary scope is evidence/test/docs only. Small `apps/admin/src/pages/knowledge/**` corrections are allowed only if browser evidence proves an obvious current React mismatch that must be fixed now. This slice does not implement KB DB/API/storage/eval runtime, does not write formal knowledge, does not publish knowledge automatically, and does not claim owner visual acceptance, runtime closure, GA-0, production deployment, real customer/order-data use, customer LLM, Telegram Business automatic reply or 1.0 release approval.
...
- Confirm this branch is stacked on #208 / `origin/codex/m7-ui-66-orders-source-parity-refresh`.
...
- Work only in `/Users/atilla/.codex/worktrees/m7-ui-67-knowledge-resources-source-parity-refresh` on branch `codex/m7-ui-67-knowledge-resources-source-parity-refresh`.
...
- `tenant.knowledge` renders inside tenant shell after selecting tenant `tenant-b` on the latest #208 stack.
...
- Any React visual corrections are small and limited to `apps/admin/src/pages/knowledge/**`.
- No disallowed files are changed.

## Validation Plan

- `git diff --check origin/codex/m7-ui-66-orders-source-parity-refresh...HEAD`
- `node scripts/guards/pr-shape.mjs --base origin/codex/m7-ui-66-orders-source-parity-refresh --spec docs/specs/M7-UI-67-knowledge-resources-source-parity-refresh.md --include-worktree`
- touched Prettier check
- ESLint on touched knowledge test/source if source changed
...
- Focused Playwright for `apps/admin/tests/m7-ui-knowledge-resources-source-parity.spec.ts` and existing `apps/admin/tests/m7-ui-knowledge-resources.spec.ts`.
