...
Implement one narrow visible-UI polish slice for `group.overview` on top of `origin/codex/m7-ui-31-orders-visible-ui`.
...
- Worker path: `/Users/atilla/.codex/worktrees/m7-ui-61-group-overview-first-viewport-cleanstack`.
...
- Preserve current visible UI trunk viewport/sidebar/topbar behavior and tenant entry baseline.
...
| Current visible UI trunk | Preserve `tenant.conversations` entry behavior when row click/keyboard enters tenant layer. |

## Implementation Plan

1. Re-read AGENTS, v1.1 source-of-truth docs, owner HTML, unpacked group source and current React group overview files/tests.
