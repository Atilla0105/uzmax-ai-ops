...
export const runtimeBoundary =
  "AI member runtime unavailable | degraded | mock | read-only | not production member metrics | no production persona publish | local action only | no DB/API closure";
...
export function gateTone(gate: PersonaGate) {
  ...
  if (gate === "pass") return ["评测通过 · 可发布预览", "ok"] as const;
...
function member(
  ...
  return {
    ...
    versions: versions.map(([version, date, note]) => ({
      date,
      note,
      text: `你是 ${name}。${note}。回复需遵循租户知识、红线、能力开关与转人工规则。`,
      version
    }))
  };
}
