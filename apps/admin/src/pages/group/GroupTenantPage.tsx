...
export function GroupTenantPage() {
  ...
  const clearToastHandle = () => {
    const handle = toastTimerRef.current;
    if (handle === null) return;
    window.clearTimeout(handle);
    toastTimerRef.current = null;
  };

  useEffect(() => clearToastHandle, []);
  ...
  return (
    ...
    >
      ...
      <ConfirmModal
        confirmLabel="确认停用"
        danger
        description="Reason required. This preview changes browser-local mock state only; no production tenant change, no tenant config persistence, no connector or feature flag change, and no audit write happens."
