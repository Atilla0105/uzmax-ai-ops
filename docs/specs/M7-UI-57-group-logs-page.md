...
Implement a UI-first `group.logs` / `集团日志` visible page on top of `origin/codex/m7-ui-31-orders-visible-ui`.
...
- Worktree: `/Users/atilla/.codex/worktrees/m7-ui-57-group-logs-page-cleanstack`.
- Branch: `codex/m7-ui-57-group-logs-page-cleanstack`.
- Base: `origin/codex/m7-ui-31-orders-visible-ui` at `e672a3c3d656371436b4061114085821351a3e2c`.
...
- Focused Playwright coverage for `activePageId=group.logs`, `shellLevel=group`, group-only nav, no tenant id, title/subtitle/runtime note, module chips, search filtering, filtered empty state, export local-only toast, detail local-only action, forced URL states via both query keys, collapsed sidebar width and mobile 320 no-overflow fallback.
- Browser evidence under `/tmp/uzmax-m7-ui-57-group-logs-visible-ui/` with desktop/table screenshot, filter-empty screenshot, mobile 320 screenshot and metrics JSON.
