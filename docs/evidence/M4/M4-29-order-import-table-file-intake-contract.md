# M4-29 Order Import Table File Intake Contract Evidence

> spec: `docs/specs/M4-29-order-import-table-file-intake-contract.md`
> branch: `codex/m4-29-order-import-table-file-intake`
> worktree: `/Users/atilla/Documents/uzmax-m4-29-order-import-table-file-intake`
> adr: `docs/adr/ADR-B02-order-api.md`
> decision_type: `order_import_table_file_intake_contract__no_storage_no_xlsx_no_runtime`
...
M4-29 adds a controlled table file intake contract for the ADR-B02 no-API import snapshot path. It exports `createOrderImportCsvTextInputFromTableFile`, which validates already-provided TSV/table text file content and metadata, strips a UTF-8 BOM if present, converts tab-separated rows into CSV text, preserves parser row budgets and returns an input compatible with the existing M4-10 `parseOrderImportCsvText` contract.
...
|---|---|---|
| Worktree path | `/Users/atilla/Documents/uzmax-m4-29-order-import-table-file-intake` | pending final validation |
| Branch | `codex/m4-29-order-import-table-file-intake` | pending final validation |
...
| TSV byte payload | UTF-8 table bytes become existing parser input | passed focused test |
| BOM and row budget | string content strips BOM and preserves `maxRows` | passed focused test |
| Fail-closed metadata/content | filename, media type, source ref, delimiter, invalid UTF-8 and byte budget fail closed | passed focused test |
...
This M4-29 slice does not delete those files. It proceeds with absolute assigned worktree paths plus CI-mode boundary check and manual root tracked/index clean evidence.

## Acceptance Mapping

| Item | Current M4 branch | M4-29 contribution |
|---|---|---|
| E-01 | `not_current_blocker__no_api_for_m4` | Adds no API connector and no `order_connector`. |
| E-02 | `table_file_intake_contract_supported_not_closed` | Adds controlled TSV/table file payload intake before the existing parser; real Storage ingestion, XLSX parser, DB client runtime/RLS integration, queue and admin E2E remain future scope. |
...
- M4-29 does not change API routes, admin UI/client, Prisma schema/migrations/generated client, production provider config, Redis connection, retry executor, Storage downloader or XLSX parser.
- M4 evidence index was synchronized to include M4-29 while preserving production DB runtime, RLS integration, queue runtime, Storage ingestion, XLSX parser, admin E2E, AI runtime/eval and release blockers.

## Review Closure

| Review | Finding | Resolution |
|---|---|---|
| Spec compliance | no issues found | Changes are limited to the M4-29 touch list and preserve no-Storage/no-XLSX/no-runtime boundaries. |
| Code quality | no issues found | Table intake reuses the existing file-intake module, fails closed, preserves parser budgets and is covered by focused tests. |
...
| Command | Result |
|---|---|
| `node --test scripts/tests/m4-order-import-table-file-intake-contract.test.mjs scripts/tests/m4-order-import-csv-file-intake-contract.test.mjs scripts/tests/m4-order-import-parser-contract.test.mjs` | passed; 12 tests |
...
| `CI=true npm run check` | passed; 258 Node tests, build, 57.17 kB brotli size limit and 11 Playwright tests |
| `node scripts/guards/pr-shape.mjs --base origin/main --spec docs/specs/M4-29-order-import-table-file-intake-contract.md --include-worktree` | passed |
