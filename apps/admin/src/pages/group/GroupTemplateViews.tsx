...
type TabChange = (id: TemplateTabId) => void;
type HeaderProps = { activeTab: TemplateTabId; onChangeTab: TabChange };
...
const focusableSelector = "button:enabled,[href],[tabindex]:not([tabindex='-1'])";

export function TemplateHeader({ activeTab, onChangeTab }: HeaderProps) {
  return (
    <header className="uz-template-head" data-testid="m7-template-header">
      ...
      <div
        aria-label="模板中心分类"
        className="uz-template-tabs"
        data-testid="m7-template-tabs"
        role="tablist"
      >
...
export function TemplateGrid({ cards, onCopy }: GridProps) {
  return (
    <section
      aria-label="模板卡片"
      className="uz-template-grid"
      data-testid="m7-template-grid"
    >
      {cards.map((card) => (
...
export function CopyModal(props: ModalProps) {
  const { card, onClose, onConfirm, onToggle, selected, selectedCount } = props;
