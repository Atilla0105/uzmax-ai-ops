...
test("proves group overview to tenant conversation visible loop", async ({ page }) => {
  ...
  await captureOwnerSourceIfAvailable(page);
...
async function captureOwnerSourceIfAvailable(page: Page) {
...
async function expectLayerNav(
  ...
  await expect
    .poll(async () => readNavigationSnapshot(page))
    .toMatchObject({
      buttonLabels: expect.arrayContaining([...visibleLabels]),
      sectionLabels: [...visibleSectionLabels]
    });

  const snapshot = await readNavigationSnapshot(page);
  expect(snapshot.sectionLabels).toEqual(visibleSectionLabels);
  expect(snapshot.sectionLabels).not.toEqual(
    expect.arrayContaining([...hiddenSectionLabels])
  );
  expect(snapshot.buttonLabels).toEqual(expect.arrayContaining([...visibleLabels]));
  expect(snapshot.buttonLabels).not.toEqual(expect.arrayContaining([...hiddenLabels]));
}

async function readNavigationSnapshot(page: Page) {
  return page.getByTestId("app-shell-nav").evaluate((nav) => {
    ...
    return {
      buttonLabels: visibleText(".uz-nav-item__label"),
      sectionLabels: visibleText(".uz-nav-group p")
    };
  });
}
