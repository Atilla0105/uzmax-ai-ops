...
- 结果：`none` / `updated` / `new doc required`。
- 判断依据：`docs/doc-gates.md`。只有 `触碰模块/文件` 或当前 repo 树命中文档触发条件时才需要详细说明；未命中写 `none`。
- 备注：本字段是 forcing function，不是权威真源；`guard:doc-triggers` 落地后以 guard 对当前 repo 树的推导为准。

## 前置条件

- 依赖账号、样本、环境、ADR 或上游 spec。
- Worktree / branch：写明预期物理 worktree 路径、branch、禁止写入的 checkout，以及开工前必须记录的 `pwd` / `git status --short --branch` / `git branch --show-current`。
- 并发派发证据：仅并发 worker 或 coordinator 派发时需要；记录 worktree 路径互斥、branch 互斥、触碰模块互斥。schema、lockfile、共享 config、CI/guard 脚本、全局生成物和 release/production 门禁改动必须全局串行。
...
- 里程碑 closeout 需要列出本里程碑 incident、清理状态、根因边界和防复发控制；没有 incident 时写 `none found in repo evidence`。
- 本 spec 若创建或更新 incident，列出 incident 文件、institutionalized 状态和证据链接。
