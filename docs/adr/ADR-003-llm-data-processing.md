...
> 日期：2026-06-14；2026-07-10 provider 事实校准（决策状态未变）
> 关联 spec：`docs/specs/M0-04-llm-data-processing-adr.md`、`docs/specs/M11-00-value0-customer-service-closure-contract.md`
...
M0 签收时唯一已准备的 provider 是 OpenAI。此后 M8-07 已加入 DeepSeek Chat Completions adapter，并在受控 staging/Test Bot 上启用 DeepSeek 路由。M8-07 只验证了官方请求/响应契约与内部测试运行；没有形成 DeepSeek 对真实客户数据的留存、区域、训练使用、abuse monitoring、数据处理协议或 owner 风险签收证据。因此 DeepSeek 与 OpenAI 一样只属于本 ADR 的 `dev_only` 范围，且不得把“已接入/已跑通”解释为真实客户数据处理已获批准。

下表保留 M0 的 OpenAI 政策审查作为历史 provider 证据。DeepSeek adapter/运行证据见 `docs/specs/M8-07-llm-composed-bot-runtime.md` 与 `docs/evidence/M8/M8-07-llm-composed-bot-runtime.md`；若要让真实客户内容进入 DeepSeek，必须先补独立官方政策审查和 owner ADR 分支签收。本 ADR 只记录公开政策与项目证据，不暴露 secret。
...
M0-04 的签收结论是：**项目 owner 选择 `dev_only` 分支。ADR 规则可作为后续实现约束；OpenAI、DeepSeek 或任何后续第三方 provider 仅允许用于合成数据、脱敏开发样本、公开知识和非客户明文任务。真实客户消息、截图、语音转写和客户档案不得进入第三方 LLM。**
...
- fallback provider 默认关闭；provider 全挂或当前 provider 不满足策略时，系统只能转人工、留单或关闭对应 AI 能力，不得切到更宽松 provider。
- OpenAI、DeepSeek 与任何后续第三方 provider 仅允许用于合成数据、脱敏开发样本、公开知识和非客户明文任务；真实客户内容必须等后续 ADR 分支更新、redaction 实装与 owner signoff。
...
| `dev_only` | owner 明确选择先不让真实客户内容进入第三方 LLM；当前 provider 接入/密钥/契约证据不构成真实客户留存、区域、DPA 或 redaction 风险签收。 | 仅允许 allowlisted 内部测试身份的合成/脱敏开发样本、公开知识和非客户明文任务；未知身份不得进入 provider。 | accepted current path；真实流量不开闸，GA-0 客户 LLM 继续阻断。 |
...
- 任一 provider 的输入/输出共享或训练使用被启用，或政策事实无法确认：立即关闭对应客户可见 LLM 路由，轮换受影响 key（如适用），记录安全事件，并重新 owner 签收。
...
- DeepSeek adapter/dev-test runtime：`docs/specs/M8-07-llm-composed-bot-runtime.md`、`docs/evidence/M8/M8-07-llm-composed-bot-runtime.md`
- M0-04 spec：`docs/specs/M0-04-llm-data-processing-adr.md`
- 签收状态：项目 owner 在 2026-06-14 选择 `dev_only` 分支；2026-07-10 仅校准当前 DeepSeek provider 事实，未改变决策。当前状态仍为 `accepted_dev_only__customer_llm_blocked`。真实客户消息、截图、语音转写和客户档案仍不得进入第三方 LLM。
