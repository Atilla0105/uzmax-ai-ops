...
Implement a UI-first `group.templates` / `模板中心` visible page on top of `origin/codex/m7-ui-42-model-cost-risk-visible-ui`.
...
- Copy-to-tenant confirmation is browser-local only. It does not persist templates, write audit, write tenant config, wire ops-assets, or close runtime/template-copy readiness.
- No owner visual acceptance, runtime closure, production template copy, tenant config/knowledge/eval/persona write, GA-0, or 1.0 release approval is claimed.
...
- Worktree: `/Users/atilla/.codex/worktrees/m7-ui-50-template-center-visible-ui`.
- Branch: `codex/m7-ui-50-template-center-visible-ui`.
- Base: `origin/codex/m7-ui-42-model-cost-risk-visible-ui`.
...
- Update `PageOutlet.tsx` to render `<GroupTemplatePage />` for `group.templates`; it must not carry `data-tenant-id`.
...
- URL query `?state=loading|empty|error|permission|degraded` and `?m7TemplateState=...` render deterministic states. Default is degraded/interactive mock.
...
- Rejected: old M5 runtime template center wiring, old `--uzmax-*` visual source, production-looking template persistence, audit/config writes, and any owner-acceptance/release/runtime closure copy because this slice is UI-first and local-only.
...
- Focused Playwright coverage for `activePageId=group.templates`, `shellLevel=group`, group-only nav, title/subtitle/tabs/card count, copy modal, disabled confirm until target selection, local-only toast with `role=status`/`aria-live`, forced URL states, collapsed sidebar width and mobile 320 no-overflow fallback.
- Browser evidence under `/tmp/uzmax-m7-ui-50-template-center-visible-ui/` with desktop screenshot, modal screenshot, mobile 320 screenshot and metrics JSON.
