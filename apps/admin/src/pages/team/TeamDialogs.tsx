...
export function MemberDrawer({ member, onClose, onSetNotify, onToggleDisable, onToggleTelegram, roleName }: { member: TeamMember | null; onClose: () => void; onSetNotify: (id: string, value: string) => void; onToggleDisable: (id: string) => void; onToggleTelegram: (id: string) => void; roleName?: string }) {
  ...
  return <section className="uz-team-drawer-scrim" data-testid="m7-team-member-drawer" onClick={onClose} role="dialog"><article aria-label="成员详情" className="uz-team-drawer" onClick={(event) => event.stopPropagation()} role="dialog"><header className="uz-team-drawer-head"><span className="uz-team-avatar" style={avatarStyle}>{member.initial}</span><div><strong>{member.name}</strong><span className="uz-team-mono">{member.account}</span></div><button className="uz-team-btn" onClick={onClose} type="button">×</button></header><div className="uz-team-drawer-body">{facts.map((line) => <p key={line}>{line}</p>)}<label>通知偏好</label><div className="uz-team-switch-row">{TEAM_NOTIFICATION_PREFS.map((pref) => <button aria-pressed={member.notifyPref === pref} className="uz-team-btn" key={pref} onClick={() => onSetNotify(member.id, pref)} type="button">{pref}</button>)}</div><div className="uz-team-toggle"><Toggle aria-label="Telegram 绑定" checked={member.telegram} onClick={() => onToggleTelegram(member.id)} /><span>Telegram 绑定 · {member.telegram ? "已绑定" : "未绑定"}</span></div><button className="uz-team-btn uz-team-btn--primary" data-testid="m7-team-member-toggle-disable" onClick={() => onToggleDisable(member.id)} type="button">{member.disabled ? "恢复账号" : "停用账号"}</button></div></article></section>;
}
