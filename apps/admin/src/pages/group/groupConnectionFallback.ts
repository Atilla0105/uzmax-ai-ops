...
export const connectionRuntimeBoundary = connectionRuntimeLabels.join(" · ");

export const connectionMeta = {
  descriptor: "集团级连接管理",
  runtime: connectionRuntimeBoundary,
  source: "centralized-synthetic-owner-fixture",
  subtitle: "集团级连接类型 · 启停/测试",
  title: "连接中心"
} as const;

export const connectionStateCopy: Record<
  Exclude<ConnectionViewState, "degraded">,
  { body: string; title: string }
> = {
  empty: {
    body: "连接类型接入后，这里会显示各渠道的覆盖租户、接入定级和最近错误。",
    title: "暂无连接类型"
  },
  error: {
    body: "连接中心暂时无法展示，请稍后刷新或联系平台管理员。",
    title: "连接中心暂不可用"
  },
  loading: {
    body: "正在同步集团连接清单、租户覆盖和最近错误。",
    title: "正在载入连接"
  },
  permission: {
    body: "当前账号需要集团管理员权限后才能查看连接中心。",
    title: "需要集团管理员权限"
  }
};
...
export function connectionToggleToast(card: ConnectionCard, enabled: boolean) {
  return `${card.title} 已在本页${enabled ? "启用" : "停用"}。连接规则接入后可同步到正式配置。`;
}

export function connectionTestToast(card: ConnectionCard) {
  return `${card.title} 复测已完成，请查看最近错误与接入定级。`;
}
