...
| 13 | M7-UI-10-confirmation-queue-page | 确认队列 | `/Users/atilla/源码/unpacked 6/pages/queue/QueuePage.tsx` | `tenant.queue` | `apps/admin/src/pages/queue/QueuePage.tsx` | `implemented_in_worker_pending_pr`; renders M7 page via existing `createConfirmationQueueApiClient`; distill health/manual recovery and keep-current remain disabled/read-only contract blockers | loading, empty, error, permission, degraded, conflict diff | `implementation_evidence_pending_pr_review` |
