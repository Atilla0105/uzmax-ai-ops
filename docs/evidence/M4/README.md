...
| E-02 | `p0_current_main_path__import_snapshot` | CSV/table import snapshot is the current order-data main path. M4-04 adds DB contracts and M4-05 adds pure row/batch contracts; runtime import/admin E2E remains future scope. |
| E-03 | `p0_remains__stale_snapshot_warning` | M4-04 adds expiry fields and M4-05 adds stale handoff contract; runtime/admin stale warning remains required. |
| E-04 | `p0_remains__no_fabricated_order_status` | M4-05 pure contract hands off missing/stale/degraded reads without status fabrication; AI eval/runtime integration remains future scope. |
...
- M4-01, M4-02, M4-04 and M4-05 are foundation/partial evidence only. They do not close the order import workflow runtime/admin/E2E blockers, stale snapshot runtime warning, customer asset API/admin flows, AI order-read runtime/eval, production readiness or release signoff.
