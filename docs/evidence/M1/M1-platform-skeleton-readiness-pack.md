...
OCM-04 进入 `ready_for_review`。M0 技术地基已达到启动 M1 平台骨架准备的前置条件：
...
本 readiness pack 不直接放行 M1 业务骨架实现。项目 owner 合并 OCM-04 PR 后，下一步必须按 Gate 1 条件做 Go/No-Go 复判；只有 Gate 1 Go 后，后续 M1 实现 spec 才能逐个开 PR。
...
| G1-6 M1 readiness pack | ready_for_review | 等待 OCM-04 PR review / merge |
| G1-7 历史样本与种子评测责任 | pending_owner_input | 本 PR 提出责任、截止时间与失败分支，待 owner 通过 merge 或 review 确认 |
| G1-8 当前 P0 残项 | pending_review | OCM-04 合并前 Gate 1 仍为 pending，不得进入 M1 实现 |

## Review Notes

- 本 readiness pack 不替代 Gate 1 Go/No-Go 记录。
- 合并本 PR 后，允许准备并逐个创建 M1 spec；实现类 PR 必须等待 Gate 1 Go。
...
| 角色 | 状态 | 备注 |
|---|---|---|
| 项目 owner | pending_pr_review | 合并 OCM-04 PR 即确认本 readiness pack 口径；若需改历史样本截止时间，应在 review/PR comment 中明确 |
| AI agent | ready_for_review | 已按 OCM-04 拆分 M1 scope、spec 队列、项目输入排期和 Gate 1 剩余状态 |
