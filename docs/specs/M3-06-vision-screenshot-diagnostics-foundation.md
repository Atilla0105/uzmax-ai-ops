...
- Screenshot diagnosis input accepts only controlled refs such as `storageRef`、`manifestRef`、`redactionRef`、`modelResultRef`、`providerResultRef`、`evalResultRef`; it rejects raw screenshot content, data URLs, public URLs, file paths, base64/blob-ish payloads, raw OCR/text/customer plaintext。
- Diagnosis candidate shape validates screenshot kind/category、required signals/checklist、confidence、evidence refs and optional model/provider/result refs as controlled refs only。
