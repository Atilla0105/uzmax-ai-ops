...
export function AgentsPage({ selectedTenantId }: { selectedTenantId: string }) {
  ...
  const actions = {
    ...
    toggleCap: (id: string, key: keyof AiMember["capabilities"]) =>
      patch(id, (agent) => {
        const enabled = !agent.capabilities[key];
        setToast(
          `${agent.name}: ${String(key)} ${enabled ? "enabled" : "disabled"}; local action only.`
        );
        return {
          ...agent,
          capabilities: { ...agent.capabilities, [key]: enabled }
        };
      })
  };
  ...
  return (
    ...
    >
      <style>{`${agentStyles}@media(max-width:720px){.uz-agent-page,.uz-agent-head,.uz-agent-note,.uz-agent-toast,.uz-agent-scroll{width:100vw;max-width:100vw}}`}</style>
...
function AgentBody({
  ...
  return (
    <main className="uz-agent-scroll">
      ...
      {filter !== "human" ? (
        <>
          <h3 className="uz-agent-section-title">AI 成员</h3>
          <AgentCards actions={actions} agents={agents} />
        </>
      ) : null}
      {filter !== "ai" ? (
        <>
          <h3 className="uz-agent-section-title">人类成员</h3>
          <HumanTable />
        </>
      ) : null}
    </main>
