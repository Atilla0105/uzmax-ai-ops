...
import { Bot, Hand, MoreHorizontal, Paperclip, SendHorizontal } from "lucide-react";
    ...
    <header className="uz-conv-thread__head">
      ...
      <div className="uz-conv-actions">
        ...
        <Button
          ...
          disabled
          icon={<IconSlot icon={MoreHorizontal} />}
...
export function ConversationRuntimeBar({ reason }: { reason: string }) {
  return (
    <div
      aria-live="polite"
      className="uz-conv-runtime-bar"
      data-testid="m7-conversation-degraded"
    >
      <StatusBadge tone="warn">degraded</StatusBadge>
      <span>{reason} 发送、人工作业外发、客户聚合和 WS 实时同步保持禁用。</span>
    </div>
  );
}
...
export function Composer({
  ...
  const draft = active?.draftText
    ? `${active.draftText}\n\nmock/read-only: Business 发送、编辑和红线复检尚未接入真实 runtime。`
    : suspended
