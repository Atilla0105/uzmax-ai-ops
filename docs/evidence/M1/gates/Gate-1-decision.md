...
2026-06-17 M1-06 复判：仓库外 seed review 共 80 条唯一记录，字段完整；意图覆盖 68 条，乌语/俄语相关语言覆盖 80 条，红线相关宽口径 54 条，其中 `restricted_goods` 25 条；phone / URL / @handle / long number / identity doc 明显敏感模式残留扫描为 0。该输入可作为 Gate 1 的 `sample_ready` 证据，允许 M1-05 后续建立正式 eval seed manifest 和 runner。

2026-06-17 M1-05 后续更新：PR #21 已建立 eval seed manifest、runner 和配额校验，当前 80 条 owner-local 聚合 seed review 通过 M1 seed quota。Gate 1 当时没有关闭 G-06；M1-05 之后 G-06 的 M1 seed quota runner/pass 已有证据，1.0 full candidate target >=200 仍按 M6 前目标跟踪。
...
| G1-7 | seed_quota_pass__content_external | `docs/evidence/M1/eval-seed/history-samples-manifest.md`、`docs/specs/M1-00-gate1-owner-input-intake.md`、`docs/specs/M1-06-gate-1-go-no-go.md`、`docs/evidence/M1/eval-seed/m1-05-seed-runner-manifest.md` | 本机真实 Telegram 导出已转成仓库外脱敏候选和 80 条 seed review；只读复核配额与明显残留通过；M1-05 runner 已通过当前聚合 seed quota；不提交样本内容到 Git |
...
| 原阻断项 | 当前状态 | 后续约束 |
|---|---|---|
| 历史咨询样本脱敏导出或明确不可提供结论 | seed_quota_pass__content_external | M1-05 已建立正式 eval seed manifest、runner 和配额校验；1.0 full candidate target >=200 仍按 M6 前目标跟踪 |
...
| G-06 状态 | Gate 1 时输入已 ready；M1-05 后 seed quota runner/pass 已有证据；1.0 full candidate target >=200 仍未关闭 |
...
- G-06 不在本 Gate 1 复判关闭；M1-05 已用 runner 和正式入集 manifest 单独记录当前 seed quota pass。
