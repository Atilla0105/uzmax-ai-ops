...
export function connectionToggleToast(card: ConnectionCard, enabled: boolean) {
  ...
  return `browser-local only: ${card.title} toggled ${state}; no production connector change, no real connection test, no audit write.`;
}

export function connectionTestToast(card: ConnectionCard) {
  return `browser-local only: ${card.title} synthetic test finished; no production connector change, no real connection test, no audit write.`;
}
