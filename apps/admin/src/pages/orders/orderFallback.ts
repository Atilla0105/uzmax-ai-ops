...
export interface OrderIconSnippets {
  package: string;
  search: string;
  triangleAlert: string;
  upload: string;
  uploadDrop: string;
  x: string;
}
...
export function renderOrdersPage(state: RenderState) {
  return `<style>${orderPageStyles}</style>${header(state)}${runtimeBar(state.icons)}${body(state)}${state.importOpen ? modal(state) : ""}`;
}
...
function body(state: RenderState) {
  ...
  return state.active
    ? detail(state.active, state.icons)
    : list(state.rows, state.viewState);
}
...
.uz-orders-modal{position:fixed;inset:0;z-index:var(--z-modal);display:grid;place-items:center;background:rgb(26 29 33 / 36%)}.uz-orders-dialog{box-sizing:border-box;width:480px;max-width:calc(100vw - 32px);border-radius:12px;padding:20px 22px;background:var(--card);box-shadow:var(--shadow-overlay)}.uz-orders-dialog-head{display:flex;align-items:center;margin-bottom:14px}.uz-orders-dialog-head strong{font:800 var(--text-title)/1.2 var(--font-display)}.uz-orders-close{display:grid;place-items:center;width:28px;height:28px;margin-left:auto;border:0;background:transparent;color:var(--ink-500);font-size:18px}.uz-orders-drop{display:grid;place-items:center;gap:6px;min-height:126px;border:1.5px dashed var(--ink-300);border-radius:10px;margin-bottom:14px;padding:18px;background:var(--paper);color:var(--ink-500);text-align:center}.uz-orders-drop strong{color:var(--ink-900)}.uz-orders-progress{height:8px;border-radius:999px;background:var(--ink-075);overflow:hidden}.uz-orders-progress span{display:block;height:100%;background:var(--ink-900)}.uz-orders-progress .is-complete{width:100%}.uz-orders-stats{display:flex;gap:20px;margin:14px 0}.uz-orders-stat span{display:block;color:var(--ink-500);font-size:10px}.uz-orders-stat strong{font:800 18px/1.2 var(--font-data)}.uz-orders-history{margin-top:16px;border-top:1px solid var(--ink-075);padding-top:12px}.uz-orders-history button{display:flex;align-items:center;gap:8px;width:100%;border:1px solid var(--ink-150);border-radius:8px;margin-top:6px;padding:8px 10px;background:var(--card);color:var(--ink-700);font:var(--text-sm)/1.3 var(--font-body);text-align:left}
