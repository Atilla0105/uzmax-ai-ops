...
export const templateRuntimeLabels =
  "degraded|mock|read-only|browser-local only|no production template copy|no audit write|no config write|no knowledge/eval/persona/quick-reply write|no ops-assets write".split(
    "|"
  );

export const templateMeta = {
  descriptor: "集团级 · mock/degraded · read-only",
  runtime: templateRuntimeLabels.join(" · "),
...
export function templateCopyLine(card: TemplateCard) {
  if (!card.copiedAt) return "尚未复制到任何租户 · synthetic read-only";
  return `最近复制 · ${card.copiedAt} · ${card.copied.length} 租户 · mock/local history`;
}

export function templateToast(card: TemplateCard, count: number, tabLabel: string) {
  return `browser-local only: ${card.name} ${card.version} -> ${count} target(s) · ${tabLabel}; no production template copy, no audit write, no config write, no knowledge/eval/persona/quick-reply write, no ops-assets write.`;
}
