...
Refresh the existing visible UI-first `tenant.tickets` / 工单 page on top of `origin/codex/m7-ui-31-orders-visible-ui` (#250 stack) so the ticket page remains aligned with the current visible UI trunk after shell/sidebar/topbar/group/tenant/conversation/queue calibrations.

This is a browser evidence/test-only source parity refresh, not a rewrite. It adds focused browser evidence for the already-present React ticket page and does not expect ticket page source changes. It does not implement ticket DB/API/runtime, does not touch shared shell/topbar/sidebar, and does not claim owner visual acceptance, runtime closure, GA-0, production deployment, real customer/order-data use, customer LLM, Telegram Business automatic reply or 1.0 release approval.
...
- Work only in `/Users/atilla/.codex/worktrees/m7-ui-64-ticket-source-parity-refresh-cleanstack` on branch `codex/m7-ui-64-ticket-source-parity-refresh-cleanstack`.
...
- source changed files: 0 expected
- source net LOC: 0 expected
- new source files: 0
- test files changed/added: <= 1 focused Playwright spec
- docs changed/added: <= 4
- package/lock/generated/config/backend/API/DB/worker/cron/CI/global config/shared shell/sidebar/topbar/ticket source/confirmation queue/customer/order/knowledge: 0
...
Owner HTML and unpacked source paths are local-only and unavailable in GitHub CI. The focused Playwright spec must capture owner/unpacked artifacts when those files exist, write explicit unavailable artifacts when they do not, and continue hard React assertions either way.
...
- `tenant.tickets` renders inside tenant shell after selecting a tenant on the current visible UI trunk.
...
- No React source change is expected. If a tiny copy/selector mismatch is exposed, stop and report before widening this test-only slice.
...
- `npm run jscpd`
- `npm run knip`
- `npm run guard:pr-shape -- --base origin/codex/m7-ui-31-orders-visible-ui --spec docs/specs/M7-UI-64-ticket-source-parity-refresh.md --include-worktree`
- `npx playwright test apps/admin/tests/m7-ui-ticket-source-parity.spec.ts apps/admin/tests/m7-ui-ticket-page.spec.ts`
- `npm run typecheck`
- `npm run lint`

## Failure Branches

- If source geometry cannot be kept without shared shell/topbar/sidebar or ticket source edits, stop and report the dependency instead of editing source in this slice.
...
- No package, lockfile, generated, CI/global config, shared AppShell/topbar/sidebar, ticket source, confirmation queue, customer, order or knowledge edits.
