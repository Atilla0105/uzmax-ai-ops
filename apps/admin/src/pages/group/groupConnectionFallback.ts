...
export const connectionRuntimeLabels =
  "degraded|mock|read-only|browser-local only|synthetic health|no production connector change|no real connection test|no external API/Telegram/order API call|no audit write|no feature flag write".split(
    "|"
  );
...
export const connectionCards: ConnectionCard[] = [
  ...
  card(
    ...
    "mock 不可用",
    "danger",
    "mock AllProvidersDown · 22分钟前",
    "mock ADR-B02：无可用 API",
    "ADR-B02",
    "无可用 API",
    false,
    "实时订单查询 · 主路径已降级为导入兜底",
...
export function connectionToggleToast(card: ConnectionCard, enabled: boolean) {
  ...
  return `browser-local only: ${card.title} toggled ${state}; no production connector change, no real connection test, no external API/Telegram/order API call, no audit write, no feature flag write.`;
}

export function connectionTestToast(card: ConnectionCard) {
  return `browser-local only: ${card.title} synthetic test finished; no production connector change, no real connection test, no external API/Telegram/order API call, no audit write, no feature flag write.`;
}
